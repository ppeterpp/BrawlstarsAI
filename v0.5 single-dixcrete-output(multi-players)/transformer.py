...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        """
        assert (len(x.shape) == 3), '{}'.format(len(x.shape))
        B, N = x.shape[:2]
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        Arguments:
            - x (:obj:`torch.Tensor`): input tensor. Shape (B, N, C), B is batch size, \
                N is number of entries(time_step), C is feature dimension(input_dim)
...
if __name__ == '__main__':
    class Net(nn.Module):
        def __init__(self):
            ...
            self.net = Transformer(6, head_num=1, output_dim=3)
        def forward(self, x, mask):
            return self.net(x, mask)

    x0 = torch.full((1,1,6), -1e9)
    x1 = torch.rand_like(x0)
    x2 = torch.zeros_like(x1)
    x3 = torch.ones_like(x1)
    xb1 = torch.cat([x3,x2,x3,x2],1).float()
    xb2 = torch.cat([x2,x2,x3,x3],1).float()
    x = torch.cat([xb1,xb2],0)
    mask1 = torch.zeros((2,1,1))
    mask2 = torch.ones((2,1,1))
    mask_ = [mask2,mask2, mask2,mask2]
    players = 3
    for i in range(4-players):
        mask_.pop(0)
        mask_.append(mask1)
    mask = torch.cat(mask_,dim=1).squeeze(2).bool()
    print(mask)
    net = Net()
    net(x, mask)